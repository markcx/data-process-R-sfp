...
#    return - object 
#############################         
readinData<-function(file, i, zip, zipT){
	#print(mode(file));
	#print(class(file));
	#print(is.character(file));
	
	if(is.character(file)){
	   obj<-process(file,i, zip, zipT) 
	   #print(obj); 
	   return(obj)			
	}
...
process<-function(file="NA", k=1, zipcode, zipT){
	    ...
	    if((i+1)<=nrow(data)){
		  if(is.numeric(data[i+1,1]) && (data[i,1]!=data[i+1,1])){
			...
			sum.OPER<-0
		        ...
		        if((data[i,2]=="ON") && data[i,6]>=0 && data[i,8]>=0 && check(data[i,5],zipcode,zipT)>0 ){
...
check<-function(pcellID, zipcode, zipTable=c()){
	print(pcellID);
	#print(zipTable);
	while(i < length(zipTable$V1)){
		if((pcellID == zipTable$V1[i]) && (zipcode==zipTable$V2[i])){
		  print(zipTable$V1[i] )
		  return(1)
		}
		i=i+1	
...
for (i in WeekDaySet){
  ...
  if(i>0){
	...
	print(filepath)
	obj<-readinData(filepath, i, as.numeric(zipPool[k]), zipTable)
	##print(obj[[1]][1]);
